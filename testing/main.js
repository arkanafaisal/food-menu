...
if(!(mq.matches)) mobileMode = false
...
async function mainFunction(){
  ...
  for(let i=0; i<data.length; i++){
    ...
    let extras = []
    for(let j=4; j<current.length; j++){
...
function initiateMainData(data){
  ...
  data.forEach(menuData => {
    ...
    const mainData = newNode.querySelector("#main-data-menu").children
    mainData[0].src = menuData.image
    mainData[0].alt = 'gambar ' + menuData.name
    mainData[0].style.maxWidth = menuListNodeWidth
    mainData[1].textContent = menuData.name
    ...
    if(menuData.stock === 'habis'){
      ...
      const button = newNode.querySelector("#place-order-button")
    ...
    for(let i=4; i<keys.length; i++){
      if(!menuData[keys[i]][1] || (menuData[keys[i]][1] && menuData[keys[i]][1].trim() === '')){
        ...
        newNode.querySelector("#detail-text").appendChild(newDetail)
        continue
      }
      ...
      for(let j=0; j<menuData[keys[i]].length; j++){
        ...
        newSelectableDetail.dataset.selfPrice = menuData.price.length > j? menuData.price[j] : menuData.price[menuData.price.length -1]
        ...
        newSelectableDetail.onclick = () => {
          ...
          if(i !== parseInt(newNode.dataset.priceRef)) return
          
          
          detailsChildren[0].textContent = "harga: " + newSelectableDetail.dataset.selfPrice
          newNode.dataset['price'] = newSelectableDetail.dataset.selfPrice
  ...
  const firstListDetail = menuList.querySelector("#detail-data-menu")
...
function changeMenuSelected(newSelected, event) {
  ...
  if(oldSelected){
    ...
    const oldDetail = oldSelected.querySelector("#detail-data-menu")
    if(mobileMode){
      oldDetail.style.maxHeight = 0
      oldDetail.style.maxWidth = oldDetail.style.scrollWidth + "px"
    }
    else {
      oldDetail.style.maxWidth = 0
      oldDetail.style.maxHeight = oldDetail.style.scrollHeight + "px"
    }
  }
  
  if(newSelected === oldSelected){return}
  newSelected.classList.add('border-3', 'border-[#FFC107]', 'menu-selected')
  const detail = newSelected.querySelector("#detail-data-menu")
  
  if(mobileMode){
    detail.style.maxHeight = detail.scrollHeight + "px"
  }
  else {
    detail.style.maxWidth = detail.scrollWidth + "px"
...
function changeMode(mobileMode){
  arr = Array.from(menuList.children)
  if(mobileMode){
    arr.forEach(val => {
      val.classList.add("flex-col")
      val.children[1].style.maxHeight = 0
    })
  } else {
    arr.forEach(val => {
      val.classList.remove("flex-col")
      val.children[1].style.maxWidth = 0
    })
  }
}
